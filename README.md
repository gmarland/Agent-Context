...
It creates symlinks inside a configurable workspace folder such as `.examples/`, keeps a sidebar list of attached folders, and can update a shared instructions file so coding agents and other collaborators know those references exist.
...
- Can automatically maintain a block in a shared instructions file listing the available examples
...
3. Updates `.gitignore` with a managed marker block that ignores the target folder
4. Optionally updates your shared instructions file with a generated section listing the attached examples
...
- Default: `AGENTS.md`
- Relative path to the shared instructions file to update
...
- The instructions sync currently appends or replaces only the managed marker block
